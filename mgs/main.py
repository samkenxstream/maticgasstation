...
def master_control(args):
    ...
    if not args.settingsFile:
        # missing settings file CLI param
        return False
    if not exists(args.settingsFile):
        # invalid path for settings file
        return False

    web3Provider = getRPC(args.settingsFile)
    # this may not be always required
    # required for talking to node, which uses PoA based
    # consensus, must for Matic nodes ( Bor node )
    setPOAMiddleWare(web3Provider)
    ...
    while True:
        try:
            ...
            blockdata.process_block_data(
                alltx.minedblock_tx_df, alltx.block_obj)
            ...
            submitted_5mago = RecentlySubmittedTxDf(
                '5mago', alltx.process_block, 10, 50, 2000000, alltx.df, txpool)
            # stats for transactions submitted ~ 30m ago
            submitted_30mago = RecentlySubmittedTxDf(
                '30mago', alltx.process_block, 60, 100, 2000000, alltx.df, txpool)
            # make a prediction table by gas price
            predictiontable = PredictionTable(
                blockdata, alltx, txpool, submitted_5mago.df, submitted_30mago.df)
            # make the gas price report
            gaspricereport = GasPriceReport(predictiontable.predictiondf, blockdata,
                                            submitted_5mago, submitted_30mago, array5m, array30m, alltx.process_block)
            # make predicted wait times
            predictiontable.get_predicted_wait(
                gaspricereport, submitted_30mago.nomine_gp)
            ...
            alltx.update_txblock(txpool.txpool_block, blockdata, predictiontable,
                                 gaspricereport.gprecs, submitted_30mago.nomine_gp)
